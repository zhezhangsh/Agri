ScorePWMWrapper <- function(pwm.set, subject.set, num.cluster=4, both.strand=TRUE, local.max=TRUE, sampling=10000, pvalue=0.01) {
...
ScorePWMPvalue <- function(score, bg) {
  ...
  up <- bg[bg >= min(score)]; 
  sc <- rev(sort(unique(score))); 
  
  ct <- sapply(sc, function(sc) length(up[up>=sc])); 
  pv <- ct/ns; 
  
  names(pv) <- sc;
  pv <- pv[as.character(score)];
  pv[pv==0] <- 1/ns; 
